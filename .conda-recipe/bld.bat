...
xcopy /S /Y /I /Q "%SRC_DIR%"\lib\MrBoxPeripheralBoard "%PREFIX%"\Library\include\Arduino\MrBoxPeripheralBoard
REM Copy compiled firmware to Conda bin directory
copy "%SRC_DIR%"\platformio.ini "%PREFIX%"\Library\bin\platformio\mr-box-peripheral-board
copy "%SRC_DIR%"\.pioenvs\default\firmware.hex "%PREFIX%"\Library\bin\platformio\mr-box-peripheral-board\default\firmware.hex
if errorlevel 1 exit 1
